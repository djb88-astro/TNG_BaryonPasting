...
soften = {'L205n2500TNG' : [0.15, 0.369],
...
class halo:
    ...
    def halo_data_store(self, mpi, subfind_table, volume, Extent=5.0, R200scale=False):
        ...
        if not mpi.Rank: print(' > Distributing particles', flush=True)
        ...
        for j in range(0, self.Nhalos, 1):
            ...
            if R200scale: Extent = self.Extent * subfind_table.R200[j]
            else:         Extent = self.Extent * ct.Mpc_cm

            # Select task to send particle data to
            destination = (j % mpi.NNodes) * Ntask_per_node + (offset % Ntask_per_node)
            if destination >= mpi.NProcs: destination -= mpi.NProcs

            if j > 0 and j % mpi.NNodes == mpi.NNodes - 1: offset += 1
            if not mpi.Rank: print('  -{0:04d} {1:03d}'.format(j, destination), flush=True)
            ...
            #--- GAS
            if 'pos' in keys:
                array = mpi.gatherv_single(np.zeros(volume.pos[gdx].shape) + volume.pos[gdx], \
                                           root=destination)
            ...
            if 'ne_nh' in keys:
                ...
                if mpi.Rank == destination: self.halo_data[subfind_table.tags[j]]['Ne_Nh'] = array
            if 'zmet' in keys:
                array = mpi.gatherv_single(volume.zmet[gdx], root=destination)
                if mpi.Rank == destination: self.halo_data[subfind_table.tags[j]]['Zmet'] = array
            if 'mass' in keys:
            ...
            if 'velc' in keys:
                array = mpi.gatherv_single(np.zeros(volume.velc[gdx].shape) + volume.velc[gdx], \
                                           root=destination)
                if mpi.Rank == destination: self.halo_data[subfind_table.tags[j]]['Velc'] = array

            #--- DM
            if 'DMpos' in keys:
                array = mpi.gatherv_single(np.zeros(volume.DMpos[ddx].shape) + volume.DMpos[ddx], \
                                           root=destination)
                if mpi.Rank == destination:
                    pos = array - subfind_table.CoP[j]
                    pos = np.where(pos > 0.5 * dims, pos - dims, pos)
            ...
            if 'DMvelc' in keys:
                ...
                if mpi.Rank == destination: self.halo_data[subfind_table.tags[j]]['DMVelc'] = array

            #--- STARS
            if 'STpos' in keys:
                array = mpi.gatherv_single(np.zeros(volume.STpos[sdx].shape, dtype=np.float) \
                                           + volume.STpos[sdx], root=destination)
                if mpi.Rank == destination:
                    pos = array - subfind_table.CoP[j]
                    pos = np.where(pos > 0.5 * dims, pos - dims, pos)
                    pos = np.where(pos < -0.5 * dims, pos + dims, pos)
            ...
            if mpi.Rank == destination:
                ...
                self.halo_data[subfind_table.tags[j]]['redshift']   = subfind_table.redshift
                self.halo_data[subfind_table.tags[j]]['rho_crit']   = subfind_table.rho_crit
                ...
                self.halo_data[subfind_table.tags[j]]['LenType']    = subfind_table.GrLenType[j]
                self.halo_data[subfind_table.tags[j]]['OffType']    = subfind_table.OffType[j]
                self.halo_data[subfind_table.tags[j]]['Nsubs']      = subfind_table.Nsubs[j]
                self.halo_data[subfind_table.tags[j]]['SubLenType'] = subfind_table.SubLenType[j]
            mpi.comm.Barrier()
        return
    ...
    def set_up_halo(self, mpi, tag):
        ...
        if 'Mass' in keys:   self.mass   = self.halo_data[tag]['Mass']
    ...
    def compute_gravitational_potentials(self, mpi):
        ...
        if not mpi.Rank: print(' > Computing gravitational potential', flush=True)
        ...
        soft = np.concatenate((np.zeros(len(self.pos), dtype=np.float) + soften[self.simtag][1],
        ...
        if not mpi.Rank: print('  -Building tree...', flush=True)
        tree = pt.construct_tree(pos, mass, soft)

        # Now compute potential
        if not mpi.Rank: print('  -Computing potential...', flush=True)
    ...
    def compute_shape_profile(self, mpi, ptype='GAS', remove_subs=True):
        ...
        if ptype in ['GAS', 'GAS_HOT', 'GAS_TEMP', 'GAS_PRES']:
            ...
            if ptype == 'GAS_HOT':
                ...
                mass = self.rho[hdx] / (ct.mu * ct.mp_g) * self.temp[hdx] \
                       * ct.kB_erg_K / ct.kev_2_erg  # [keV/cm^3]
        elif ptype == 'DM':
            pos  = np.copy(self.DMpos) / ct.Mpc_cm
            mass = np.copy(self.DMmass) / ct.Msun_g
        elif ptype == 'STAR':
    ...
    def compute_mass_profiles(self, mpi, Nb=25):
        ...
        if not mpi.Rank: print(' > Computing mass profiles', flush=True)
        ...
        if 'rad' in self.__dict__.keys():
            ...
            if 'temp' in self.__dict__.keys():
                hdx = np.where((self.temp > 1.0e6) & (self.rho * 0.752 / ct.mp_g < 0.1))[0]
                self.GHOTpro = np.histogram(self.rad[hdx], bins=self.bins, \
                                            weights=self.mass[hdx])[0]
        if 'DMrad' in self.__dict__.keys():
            self.DMpro  = np.histogram(self.DMrad, bins=self.bins, weights=self.DMmass)[0]
        if 'STrad' in self.__dict__.keys():
            self.STpro  = np.histogram(self.STrad, bins=self.bins, weights=self.STmass)[0]

        # Temperature profile
        if 'rad' in self.__dict__.keys():
            self.TEMPpro = np.histogram(self.rad, bins=self.bins, weights=self.mass * self.temp)[0]
        ...
        if 'rad' in self.__dict__.keys(): self.TOTALpro += self.GASpro
        if 'DMrad' in self.__dict__.keys(): self.TOTALpro += self.DMpro
        if 'STrad' in self.__dict__.keys(): self.TOTALpro += self.STpro
        if self.TOTALpro.sum() <= 0.0: del self.TOTALpro
        return
    ...
    def compute_velocities_and_non_thermal_pressure(self, mpi, Nb=25):
        ...
        # GAS
        if 'rad' in self.__dict__.keys():
            vr_g = (self.pos * Gvel).sum(axis=-1) / np.sqrt((self.pos ** 2.0).sum(axis=-1))
            vt_g = (Gvel[:,0] * self.pos[:,1] - self.pos[:,0] * Gvel[:,1]) \
                   / np.sqrt((self.pos[:,0:2] ** 2.0).sum(axis=-1))
            vp_g = (self.pos[:,2] * (self.pos[:,0] * Gvel[:,0] + self.pos[:,1] * Gvel[:,1]) \
                    - Gvel[:,2] * (self.pos[:,0:2] ** 2.0).sum(axis=-1)) \
                / (np.sqrt((self.pos ** 2.0).sum(axis=-1)) \
                   * np.sqrt((self.pos[:,0:2] ** 2.0).sum(axis=-1)))
        # DM
        if 'DMrad' in self.__dict__.keys():
            vr_d = (self.DMpos * Dvel).sum(axis=-1) / np.sqrt((self.DMpos ** 2.0).sum(axis=-1))
            vt_d = (Dvel[:,0] * self.DMpos[:,1] - self.DMpos[:,0] * Dvel[:,1]) \
                   / np.sqrt((self.DMpos[:,0:2] ** 2.0).sum(axis=-1))
            vp_d = (self.DMpos[:,2] * (self.DMpos[:,0] * Dvel[:,0] + self.DMpos[:,1] * Dvel[:,1]) \
                    - Dvel[:,2] * (self.DMpos[:,0:2] ** 2.0).sum(axis=-1)) \
                / (np.sqrt((self.DMpos ** 2.0).sum(axis=-1)) \
                   * np.sqrt((self.DMpos[:,0:2] ** 2.0).sum(axis=-1)))
        # STARS
        if 'STrad' in self.__dict__.keys():
            vr_s = (self.STpos * Svel).sum(axis=-1) / np.sqrt((self.STpos ** 2.0).sum(axis=-1))
            vt_s = (Svel[:,0] * self.STpos[:,1] - self.STpos[:,0] * Svel[:,1]) \
                   / np.sqrt((self.STpos[:,0:2] ** 2.0).sum(axis=-1))
            vp_s = (self.STpos[:,2] * (self.STpos[:,0] * Svel[:,0] + self.STpos[:,1] * Svel[:,1]) \
        ...
        if 'rad' in self.__dict__.keys():
            ...
            self.vr_gas = np.histogram(self.rad, bins=self.bins, weights=self.mass * vr_g)[0]
            self.vt_gas = np.histogram(self.rad, bins=self.bins, weights=self.mass * vt_g)[0]
            self.vp_gas = np.histogram(self.rad, bins=self.bins, weights=self.mass * vp_g)[0]
        ...
        if 'DMrad' in self.__dict__.keys():
            ...
            self.vr_dm = np.histogram(self.DMrad, bins=self.bins, weights=self.DMmass * vr_d)[0]
            self.vt_dm = np.histogram(self.DMrad, bins=self.bins, weights=self.DMmass * vt_d)[0]
            self.vp_dm = np.histogram(self.DMrad, bins=self.bins, weights=self.DMmass * vp_d)[0]
        ...
        if 'STrad' in self.__dict__.keys():
            ...
            self.vr_star = np.histogram(self.STrad, bins=self.bins, weights=self.STmass * vr_s)[0]
            self.vt_star = np.histogram(self.STrad, bins=self.bins, weights=self.STmass * vt_s)[0]
            self.vp_star = np.histogram(self.STrad, bins=self.bins, weights=self.STmass * vp_s)[0]
        ...
        if 'rad' in self.__dict__.keys():
            ...
            for j in np.unique(gdx):
                if j >= 0 and j < Nb:
                    self.sigr_gas[j] = np.sqrt(np.average((vr_g[gdx == j] - self.vr_gas[j]) ** 2.0, weights=self.mass[gdx == j])) / ct.km_cm
                    self.sigt_gas[j] = np.sqrt(np.average((vt_g[gdx == j] - self.vt_gas[j]) ** 2.0, weights=self.mass[gdx == j])) / ct.km_cm
                    self.sigp_gas[j] = np.sqrt(np.average((vp_g[gdx == j] - self.vp_gas[j]) ** 2.0, weights=self.mass[gdx == j])) / ct.km_cm
            del gdx
        # DM
        if 'DMrad' in self.__dict__.keys():
            ...
            for j in np.unique(ddx):
                if j >= 0 and j < Nb:
                    self.sigr_dm[j] = np.sqrt(np.average((vr_d[ddx == j] - self.vr_dm[j]) ** 2.0, \
                                                         weights=self.DMmass[ddx == j])) / ct.km_cm
                    self.sigt_dm[j] = np.sqrt(np.average((vt_d[ddx == j] - self.vt_dm[j]) ** 2.0, \
                                                         weights=self.DMmass[ddx == j])) / ct.km_cm
                    self.sigp_dm[j] = np.sqrt(np.average((vp_d[ddx == j] - self.vp_dm[j]) ** 2.0, \
                                                         weights=self.DMmass[ddx == j])) / ct.km_cm
            del ddx
        # STARS
        if 'STrad' in self.__dict__.keys():
            ...
            for j in np.unique(sdx):
                if j >= 0 and j < Nb:
                    self.sigr_star[j] = np.sqrt(np.average((vr_s[sdx == j] - self.vr_star[j]) ** 2.0, weights=self.STmass[sdx == j])) / ct.km_cm
                    self.sigt_star[j] = np.sqrt(np.average((vt_s[sdx == j] - self.vt_star[j]) ** 2.0, weights=self.STmass[sdx == j])) / ct.km_cm
                    self.sigp_star[j] = np.sqrt(np.average((vp_s[sdx == j] - self.vp_star[j]) ** 2.0, weights=self.STmass[sdx == j])) / ct.km_cm
            del sdx

        # Non-thermal pressure
        self.vols = (4.0 / 3.0) * np.pi \
                    * ((self.bins[1:] * self.R200) ** 3.0 - (self.bins[:-1] * self.R200) ** 3.0)
        if 'rad' in self.__dict__.keys():
            rho   = self.GASpro / self.vols
            vmag2 = (self.sigr_gas ** 2.0 + self.sigt_gas ** 2.0 + self.sigp_gas ** 2.0) * 1.0e10
    ...
    def compute_thermo_profiles(self, mpi, Nb=25):
        ...
        if not mpi.Rank: print(' > Computing thermodynamic profiles', flush=True)
        ...
        mass_hot     = np.histogram(self.rad[idx], bins=self.bins, weights=self.mass[idx])[0]
        hdx          = np.where(mass_hot > 0.0)[0]
        self.Rho_hot = (mass_hot / self.vols) * (ct.Mpc_cm ** 3.0 / ct.Msun_g) # [Msun / Mpc^3]

        # Spectroscopic-like temperature profile
        self.Tsl      = np.zeros(len(self.cens), dtype=np.float64)
        wgts1         = np.histogram(self.rad[idx], bins=self.bins, weights=((self.rho[idx] * (ct.Mpc_cm ** 3.0) / ct.Msun_g) ** 2.0) / (self.temp[idx] ** 0.5))[0]
        wgts2         = np.histogram(self.rad[idx], bins=self.bins, weights=((self.rho[idx] * (ct.Mpc_cm ** 3.0) / ct.Msun_g) ** 2.0) / (self.temp[idx] ** 1.5))[0]
        self.Tsl[hdx] = (ct.kB_erg_K / ct.kev_2_erg) * (wgts1[hdx] / wgts2[hdx]) # [keV]

        # Clumping density profiles
        mass = np.histogram(self.rad, bins=self.bins, weights=self.mass)[0]
        mdx  = np.where(mass > 0.0)[0]

        self.rho_sq = np.histogram(self.rad, bins=self.bins, \
                                   weights=self.mass * self.rho * self.rho)[0]
        self.rho_sq[mdx] /= mass[mdx] * (ct.Msun_g ** 2.0 / ct.Mpc_cm ** 6.0) # [Msun^2 / Mpc^6]

        self.rho_sq_hot       = np.histogram(self.rad[idx], bins=self.bins, \
                                             weights=self.mass[idx] * self.rho[idx] \
                                             * self.rho[idx])[0]
        self.rho_sq_hot[hdx] /= mass_hot[hdx] \
                                * (ct.Msun_g ** 2.0 / ct.Mpc_cm ** 6.0) # [Msun^2 / Mpc^6]

        # Clumping pressure profiles
        pres = ct.kB_erg_K * self.temp * self.rho / (ct.mu * ct.mp_g)

        self.pres_sq       = np.histogram(self.rad, bins=self.bins, \
                                          weights=self.mass * pres * pres)[0]
        self.pres_sq[mdx] /= mass[mdx] # [erg^2 / cm^6]

        self.pres_sq_hot       = np.histogram(self.rad[idx], bins=self.bins, \
                                              weights=self.mass[idx] * pres[idx] * pres[idx])[0]
        ...
        sp_wgt         = np.histogram(self.rad[idx], bins=self.bins, weights=(self.rho[idx] * (ct.Mpc_cm ** 3.0 / ct.Msun_g) ** 2.0) * (self.temp[idx] ** 0.5))[0]
        self.emm       = np.histogram(self.rad[idx], bins=self.bins, weights=(self.rho[idx] * (ct.Mpc_cm ** 3.0 / ct.Msun_g) ** 2.0) * (self.temp[idx] ** 0.5) * ne)[0]
    ...
    def compute_metallicity_profiles(self, mpi, Nb=25):
        ...
        if not mpi.Rank: print(' > Computing metallicity profiles', flush=True)
        ...
        sp_wgt          = np.histogram(self.rad[idx], bins=self.bins, weights=(self.rho[idx] * (ct.Mpc_cm ** 3.0 / ct.Msun_g) ** 2.0) * (self.temp[idx] ** 0.5))[0]
        self.Zgas       = np.histogram(self.rad[idx], bins=self.bins, weights=(self.rho[idx] * (ct.Mpc_cm ** 3.0 / ct.Msun_g) ** 2.0) * (self.temp[idx] ** 0.5) * self.zmet[idx])[0]
        edx             = np.where(sp_wgt > 0.0)[0]
        ...
        self.Zstar       = np.histogram(self.STrad, bins=self.bins, weights=self.STmass \
                                        * self.STzmet)[0]
    ...
    def compute_observable_properties(self, mpi, Emin=0.5, Emax=2.0, Enorm=65.0):
        ...
        if not mpi.Rank: print(' > Computing observable properties', flush=True)
        ...
        #--- Compton-y signal
        Ysz = (ct.sigT / (ct.me_keV * ct.kev_2_erg)) * ct.kB_erg_K * self.temp[hdx] \
              * (self.mass[hdx] * 0.752 * self.ne_nh[hdx] / ct.mp_g) / ct.Mpc_cm ** 2.0
        ...
        ne2dV = (self.mass[hdx] / self.rho[hdx]) * \
                (self.ne_nh[hdx] * 0.76 * self.rho[hdx] / ct.mp_g) ** 2.0
        ...
        self.Lx_500ce = Lx_sft[(self.rad[hdx] > 0.15 * self.R500 / self.R200)
                                & (self.rad[hdx] <= self.R500 / self.R200)].sum()
        ...
        for j in np.unique(idx):
            ...
            T = np.log10(np.sum((self.mass[hdx] * self.temp[hdx])[idx == j]) \
                         / np.sum((self.mass[hdx])[idx == j]))
            D = np.log10(np.sum((self.mass[hdx] * ne2dV)[idx == j]) \
                         / np.sum((self.mass[hdx])[idx == j])) - self.Enorm
            Z = np.sum((self.mass[hdx] * self.zmet[hdx])[idx == j]) \
                / np.sum((self.mass[hdx])[idx == j])

            # Fit spectrum for temperature
            try:
                limits = ([T - 0.5, -6.0, 1.0e-6], [T + 0.5, 6.0, 10.0])
                fit    = least_squares(self._spectrum_model, [T, D, Z], \
                                       args=(Xspec_ann, 'FIT_LS'), bounds=limits, method='trf')
            except:
                limits = [(T - 0.5, T + 0.5), (-6.0, 6.0), (1.0e-6, 10.0)]
                fit    = minimize(self._spectrum_model, [T, D, Z], args=(Xspec_ann, 'FIT_MN'), \
                                  bounds=limits, method='TNC', options={'maxiter' : 200})
            self.Tx_pro[j] = (ct.kB_erg_K / ct.kev_2_erg) * 10.0 ** fit.x[0]

        #--- Tx apertures
        # R500
        Xspec_ann = Xspec[self.rad[hdx] <= self.R500 / self.R200].sum(axis=0) * self.photon_conv

        T = np.log10(np.sum((self.mass[hdx] * self.temp[hdx])[self.rad[hdx] <= self.R500 / self.R200]) / np.sum((self.mass[hdx])[self.rad[hdx] <= self.R500 / self.R200]))
        D = np.log10(np.sum((self.mass[hdx] * ne2dV)[self.rad[hdx] <= self.R500 / self.R200]) / np.sum((self.mass[hdx])[self.rad[hdx] <= self.R500 / self.R200])) - self.Enorm
        Z = np.sum((self.mass[hdx] * self.zmet[hdx])[self.rad[hdx] <= self.R500 / self.R200]) / np.sum((self.mass[hdx])[self.rad[hdx] <= self.R500 / self.R200])
        ...
        # R500 CE
        Xspec_ann = Xspec[(self.rad[hdx] <= 0.15 * self.R500 / self.R200) & (self.rad[hdx] <= self.R500 / self.R200)].sum(axis=0) * self.photon_conv

        T = np.log10(np.sum((self.mass[hdx] * self.temp[hdx])[(self.rad[hdx] <= 0.15 * self.R500 / self.R200) & (self.rad[hdx] <= self.R500 / self.R200)]) / np.sum((self.mass[hdx])[(self.rad[hdx] <= 0.15 * self.R500 / self.R200) & (self.rad[hdx] <= self.R500 / self.R200)]))
        D = np.log10(np.sum((self.mass[hdx] * ne2dV)[(self.rad[hdx] <= 0.15 * self.R500 / self.R200) & (self.rad[hdx] <= self.R500 / self.R200)]) / np.sum((self.mass[hdx])[(self.rad[hdx] <= 0.15 * self.R500 / self.R200) & (self.rad[hdx] <= self.R500 / self.R200)])) - self.Enorm
        Z = np.sum((self.mass[hdx] * self.zmet[hdx])[(self.rad[hdx] <= 0.15 * self.R500 / self.R200) & (self.rad[hdx] <= self.R500 / self.R200)]) / np.sum((self.mass[hdx])[(self.rad[hdx] <= 0.15 * self.R500 / self.R200) & (self.rad[hdx] <= self.R500 / self.R200)])
        ...
        T = np.log10(np.sum((self.mass[hdx] * self.temp[hdx])[self.rad[hdx] <=1.0]) / np.sum((self.mass[hdx])[self.rad[hdx] <= 1.0]))
        D = np.log10(np.sum((self.mass[hdx] * ne2dV)[self.rad[hdx] <= 1.0]) / np.sum((self.mass[hdx])[self.rad[hdx] <= 1.0])) - self.Enorm
        Z = np.sum((self.mass[hdx] * self.zmet[hdx])[self.rad[hdx] <= 1.0]) / np.sum((self.mass[hdx])[self.rad[hdx] <= 1.0])
    ...
    def compute_centre_of_mass_offset(self, mpi):
        ...
        if not mpi.Rank: print(' > Computing centre of mass offset', flush=True)
        # Initialise
        self.Xoff = 0.0
        gmass     = 0.0
        dmass     = 0.0
    ...
    def compute_concentration(self, mpi, Nb=25):
        ...
        if not mpi.Rank: print(' > Computing concentration', flush=True)

        if 'TOTALpro' not in self.__dict__.keys(): self.compute_mass_profiles(mpi, Nb=Nb)

        # Fit NFW profile
        p0  = [6.0]
        try:
            fit = least_squares(self.fit_NFW, p0, args=(0, 'FIT_LS'), bounds=([0.1, np.inf]), \
                                method='trf')
        except:
    ...
    def fit_NFW(self, p0, dum, mode):
        ...
        if mode == 'MODEL':
            ...
            return np.sum((np.log10(model[idx]) \
                           - np.log10(self.TOTALpro[idx] / self.vols[idx])) ** 2.0)
        return
    ...
    def compute_DM_surface_pressure(self, mpi):
        ...
        if not mpi.Rank: print(' > Computing dark matter surface pressure', flush=True)
    ...
    def compute_kinetic_and_potential_energies(self, mpi):
        ...
        if not mpi.Rank: print(' > Computing halo kinetic and potential energies', flush=True)
    ...
    def set_up_radial_profile(self, rmin=0.05, rmax=5.0, logb=True):
        ...
        if logb:
            ...
            self.cens = 10.0 ** (0.5 * (np.log10(self.bins[1:]) + np.log10(self.bins[:-1])))
        else:
    ...
    def save(self, mpi):
        ...
        if not mpi.Rank: print(' > Saving halo', flush=True)
        ftag = 'output/{0}_Snap{1:03d}_z{2:d}p{3:02d}_{4:03d}.hdf5'.format(self.fname, self.snap, int(self.redshift), int(100.0 * (self.redshift - int(self.redshift))), mpi.Rank)
        ...
        if self.tag not in f.keys():
            ...
            # Halo properties
            if 'M500' in keys:     grp.attrs['M500_Msun'] = self.M500 / ct.Msun_g
            if 'R500' in keys:     grp.attrs['R500_Mpc']  = self.R500 / ct.Mpc_cm
            if 'M200' in keys:     grp.attrs['M200_Msun'] = self.M200 / ct.Msun_g
            if 'R200' in keys:     grp.attrs['R200_Mpc']  = self.R200 / ct.Mpc_cm
            if 'Rvir' in keys:     grp.attrs['Rvir_Mpc']  = self.Rvir / ct.Mpc_cm
            if 'redshift' in keys: grp.attrs['Redshift']  = self.redshift
            # Morphology statistics
            if 'Cvir' in keys: grp.attrs['Cvir']          = self.Cvir
            if 'Xoff' in keys: grp.attrs['Xoff']          = self.Xoff
            if 'Tkin' in keys: grp.attrs['Tkin_erg']      = self.Tkin
            if 'Upot' in keys: grp.attrs['Upot_erg']      = self.Upot
            if 'Pdm' in keys:  grp.attrs['Pdm_erg_cm^-3'] = self.Pdm
            # Observable properties
            if 'Lx_500' in keys:    grp.attrs['Lx500_erg_s^-1']   = self.Lx_500
            if 'Lx_500ce' in keys:  grp.attrs['Lx500ce_erg_s^-1'] = self.Lx_500ce
            if 'Lx_200' in keys:    grp.attrs['Lx200_erg_s^-1']   = self.Lx_200
            if 'Tx_500' in keys:    grp.attrs['Tx500_keV']        = self.Tx_500
            if 'Tx_500ce' in keys:  grp.attrs['Tx500ce_keV']      = self.Tx_500ce
            if 'Tx_200' in keys:    grp.attrs['Tx200_keV']        = self.Tx_200
            if 'Ysz_500' in keys:   grp.attrs['Ysz500_Mpc^2']     = self.Ysz_500
            if 'Ysz_200' in keys:   grp.attrs['Ysz200_Mpc^2']     = self.Ysz_200
            if 'Ysz_5r500' in keys: grp.attrs['Ysz5r500_Mpc^2']   = self.Ysz_5r500
            # Cumulative shape measures
            if 'q_dm_200' in keys:  grp.attrs['Qdm_200']   = self.q_dm_200
            if 'q_dm_500' in keys:  grp.attrs['Qdm_500']   = self.q_dm_500
            if 'q_dm_vir' in keys:  grp.attrs['Qdm_vir']   = self.q_dm_vir
            if 'q_gas_200' in keys: grp.attrs['Qgas_200']  = self.q_gas_200
            if 'q_gas_500' in keys: grp.attrs['Qgas_500']  = self.q_gas_500
            if 'q_gas_vir' in keys: grp.attrs['Qgas_vir']  = self.q_gas_vir
            if 'q_st_200' in keys:  grp.attrs['Qstar_200'] = self.q_st_200
            if 'q_st_500' in keys:  grp.attrs['Qstar_500'] = self.q_st_500
            if 'q_st_vir' in keys:  grp.attrs['Qstar_vir'] = self.q_st_vir
            if 's_dm_200' in keys:  grp.attrs['Sdm_200']   = self.s_dm_200
            if 's_dm_500' in keys:  grp.attrs['Sdm_500']   = self.s_dm_500
            if 's_dm_vir' in keys:  grp.attrs['Sdm_vir']   = self.s_dm_vir
            if 's_gas_200' in keys: grp.attrs['Sgas_200']  = self.s_gas_200
            if 's_gas_500' in keys: grp.attrs['Sgas_500']  = self.s_gas_500
            if 's_gas_vir' in keys: grp.attrs['Sgas_vir']  = self.s_gas_vir
            if 's_st_200' in keys:  grp.attrs['Sstar_200'] = self.s_st_200
            if 's_st_500' in keys:  grp.attrs['Sstar_500'] = self.s_st_500
            if 's_st_vir' in keys:  grp.attrs['Sstar_vir'] = self.s_st_vir

        #--- Profiles
        # Basics
        if 'cens' in keys:
            if 'Radii_Mpc' in fk: del f['{0}/Radii_Mpc'.format(self.tag)]
            f.create_dataset('{0}/Radii_Mpc'.format(self.tag), \
                             data=self.cens * self.R200 / ct.Mpc_cm)
        if 'vols' in keys:
            if 'Volumes_Mpc^3' in fk: del f['{0}/Volumes_Mpc^3'.format(self.tag)]
            f.create_dataset('{0}/Volumes_Mpc^3'.format(self.tag), \
                             data=self.vols / ct.Mpc_cm ** 3.0)
        # Mass profiles
        if 'GASpro' in keys:
            if 'Mgas_Msun' in fk: del f['{0}/Mgas_Msun'.format(self.tag)]
            f.create_dataset('{0}/Mgas_Msun'.format(self.tag), data=self.GASpro / ct.Msun_g)
        if 'GHOTpro' in keys:
            if 'Mgas_hot_Msun' in fk: del f['{0}/Mgas_hot_Msun'.format(self.tag)]
            f.create_dataset('{0}/Mgas_hot_Msun'.format(self.tag), data=self.GHOTpro / ct.Msun_g)
        if 'DMpro' in keys:
            if 'Mdm_Msun' in fk: del f['{0}/Mdm_Msun'.format(self.tag)]
            f.create_dataset('{0}/Mdm_Msun'.format(self.tag), data=self.DMpro / ct.Msun_g)
        if 'STpro' in keys:
            if 'Mstars_Msun' in fk: del f['{0}/Mstars_Msun'.format(self.tag)]
            f.create_dataset('{0}/Mstars_Msun'.format(self.tag), data=self.STpro / ct.Msun_g)
        # Thermodynamic profiles
        if 'TEMPpro' in keys:
            if 'Tmw_keV' in fk: del f['{0}/Tmw_keV'.format(self.tag)]
            f.create_dataset('{0}/Tmw_keV'.format(self.tag), data=(ct.kB_erg_K / ct.kev_2_erg) \
                             * self.TEMPpro)
        if 'Tsl' in keys:
            if 'Tsl_keV' in fk: del f['{0}/Tsl_keV'.format(self.tag)]
            f.create_dataset('{0}/Tsl_keV'.format(self.tag), data=self.Tsl)
        if 'rho_sq' in keys:
            if 'RHO_sq_Msun^2_Mpc^-6' in fk: del f['{0}/RHO_sq_Msun^2_Mpc^-6'.format(self.tag)]
            f.create_dataset('{0}/RHO_sq_Msun^2_Mpc^-6'.format(self.tag), data=self.rho_sq)
        if 'rho_sq_hot' in keys:
            if 'RHO_sq_hot_Msun^2_Mpc^-6' in fk:
                del f['{0}/RHO_sq_hot_Msun^2_Mpc^-6'.format(self.tag)]
            f.create_dataset('{0}/RHO_sq_hot_Msun^2-Mpc^-6'.format(self.tag), data=self.rho_sq_hot)
        if 'pres_sq' in keys:
            if 'P_sq_erg^2_cm^-6' in fk: del f['{0}/P_sq_erg^2_cm^-6'.format(self.tag)]
            f.create_dataset('{0}/P_sq_erg^2_cm^-6'.format(self.tag), data=self.pres_sq)
        if 'pres_sq_hot' in keys:
            if 'P_sq_hot_erg^2_cm^-6' in fk: del f['{0}/P_sq_hot_erg^2_cm^-6'.format(self.tag)]
            f.create_dataset('{0}/P_sq_hot_erg^2_cm^-6'.format(self.tag), data=self.pres_sq_hot)
        if 'Pkin' in keys:
            if 'Pnth_erg_cm^-3' in fk: del f['{0}/Pnth_erg_cm^-3'.format(self.tag)]
            f.create_dataset('{0}/Pnth_erg_cm^-3'.format(self.tag), data=self.Pkin)
        # Emission measure profile
        if 'emm' in keys:
            if 'EMM_cm^-6' in fk: del f['{0}/EMM_cm^-6'.format(self.tag)]
            f.create_dataset('{0}/EMM_cm^-6'.format(self.tag), data=self.emm)
        # Observable properties
        if 'Lx_pro' in keys:
            if 'Lx_pro_erg_s^-1' in fk: del f['{0}/Lx_pro_erg_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Lx_pro_erg_s^-1'.format(self.tag), data=self.Lx_pro)
        if 'Tx_pro' in keys:
            if 'Tx_pro_keV' in fk: del f['{0}/Tx_pro_keV'.format(self.tag)]
            f.create_dataset('{0}/Tx_pro_keV'.format(self.tag), data=self.Tx_pro)
        if 'Ysz_pro' in keys:
            if 'Ysz_pro_Mpc^2' in fk: del f['{0}/Ysz_pro_Mpc^2'.format(self.tag)]
            f.create_dataset('{0}/Ysz_pro_Mpc^2'.format(self.tag), data=self.Ysz_pro)
        # Metallicity profiles
        if 'Zgas' in keys:
            if 'Zmet_gas_solar' in fk: del f['{0}/Zmet_gas_solar'.format(self.tag)]
            f.create_dataset('{0}/Zmet_gas_solar'.format(self.tag), data=self.Zgas)
        if 'Zstar' in keys:
            if 'Zmet_star_solar' in fk: del f['{0}/Zmet_star_solar'.format(self.tag)]
            f.create_dataset('{0}/Zmet_star_solar'.format(self.tag), data=self.Zstar)
        # Velocity profiles
        if 'vr_gas' in keys:
            if 'Vr_gas_km_s^-1' in fk: del f['{0}/Vr_gas_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vr_gas_km_s^-1'.format(self.tag), data=self.vr_gas)
        if 'vt_gas' in keys:
            if 'Vt_gas_km_s^-1' in fk: del f['{0}/Vt_gas_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vt_gas_km_s^-1'.format(self.tag), data=self.vt_gas)
        if 'vp_gas' in keys:
            if 'Vp_gas_km_s^-1' in fk: del f['{0}/Vp_gas_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vp_gas_km_s^-1'.format(self.tag), data=self.vp_gas)
        if 'vr_dm' in keys:
            if 'Vr_dm_km_s^-1' in fk: del f['{0}/Vr_dm_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vr_dm_km_s^-1'.format(self.tag), data=self.vr_dm)
        if 'vt_dm' in keys:
            if 'Vt_dm_km_s^-1' in fk: del f['{0}/Vt_dm_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vt_dm_km_s^-1'.format(self.tag), data=self.vt_dm)
        if 'vp_dm' in keys:
            if 'Vp_dm_km_s^-1' in fk: del f['{0}/Vp_dm_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vp_dm_km_s^-1'.format(self.tag), data=self.vp_dm)
        if 'vr_star' in keys:
            if 'Vr_star_km_s^-1' in fk: del f['{0}/Vr_star_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vr_star_km_s^-1'.format(self.tag), data=self.vr_star)
        if 'vt_star' in keys:
            if 'Vt_star_km_s^-1' in fk: del f['{0}/Vt_star_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vt_star_km_s^-1'.format(self.tag), data=self.vt_star)
        if 'vp_star' in keys:
            if 'Vp_star_km_s^-1' in fk: del f['{0}/Vp_star_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/Vp_star_km_s^-1'.format(self.tag), data=self.vp_star)
        # Dispersion profiles
        if 'sigr_gas' in keys:
            if 'SIGr_gas_km_s^-1' in fk: del f['{0}/SIGr_gas_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGr_gas_km_s^-1'.format(self.tag), data=self.sigr_gas)
        if 'sigt_gas' in keys:
            if 'SIGt_gas_km_s^-1' in fk: del f['{0}/SIGt_gas_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGt_gas_km_s^-1'.format(self.tag), data=self.sigt_gas)
        if 'sigp_gas' in keys:
            if 'SIGp_gas_km_s^-1' in fk: del f['{0}/SIGp_gas_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGp_gas_km_s^-1'.format(self.tag), data=self.sigp_gas)
        if 'sigr_dm' in keys:
            if 'SIGr_dm_km_s^-1' in fk: del f['{0}/SIGr_dm_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGr_dm_km_s^-1'.format(self.tag), data=self.sigr_dm)
        if 'sigt_dm' in keys:
            if 'SIGt_dm_km_s^-1' in fk: del f['{0}/SIGt_dm_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGt_dm_km_s^-1'.format(self.tag), data=self.sigt_dm)
        if 'sigp_dm' in keys:
            if 'SIGp_dm_km_s^-1' in fk: del f['{0}/SIGp_dm_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGp_dm_km_s^-1'.format(self.tag), data=self.sigp_dm)
        if 'sigr_star' in keys:
            if 'SIGr_star_km_s^-1' in fk: del f['{0}/SIGr_star_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGr_star_km_s^-1'.format(self.tag), data=self.sigr_star)
        if 'sigt_star' in keys:
            if 'SIGt_star_km_s^-1' in fk: del f['{0}/SIGt_star_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGt_star_km_s^-1'.format(self.tag), data=self.sigt_star)
        if 'sigp_star' in keys:
            if 'SIGp_star_km_s^-1' in fk: del f['{0}/SIGp_star_km_s^-1'.format(self.tag)]
            f.create_dataset('{0}/SIGp_star_km_s^-1'.format(self.tag), data=self.sigp_star)
        # Shape profiles
        if 'q_gas' in keys:
            if 'Qgas' in fk: del f['{0}/Qgas'.format(self.tag)]
            f.create_dataset('{0}/Qgas'.format(self.tag), data=self.q_gas)
        if 's_gas' in keys:
            if 'Sgas' in fk: del f['{0}/Sgas'.format(self.tag)]
            f.create_dataset('{0}/Sgas'.format(self.tag), data=self.s_gas)
        if 'q_ghot' in keys:
            if 'Qgas_hot' in fk: del f['{0}/Qgas_hot'.format(self.tag)]
            f.create_dataset('{0}/Qgas_hot'.format(self.tag), data=self.q_ghot)
        if 's_ghot' in keys:
            if 'Sgas_hot' in fk: del f['{0}/Sgas_hot'.format(self.tag)]
            f.create_dataset('{0}/Sgas_hot'.format(self.tag), data=self.s_ghot)
        if 'q_temp' in keys:
            if 'Qtemp' in fk: del f['{0}/Qtemp'.format(self.tag)]
            f.create_dataset('{0}/Qtemp'.format(self.tag), data=self.q_temp)
        if 's_temp' in keys:
            if 'Stemp' in fk: del f['{0}/Stemp'.format(self.tag)]
            f.create_dataset('{0}/Stemp'.format(self.tag), data=self.s_temp)
        if 'q_pres' in keys:
            if 'Qpres' in fk: del f['{0}/Qpres'.format(self.tag)]
            f.create_dataset('{0}/Qpres'.format(self.tag), data=self.q_pres)
        if 's_pres' in keys:
            if 'Spres' in fk: del f['{0}/Spres'.format(self.tag)]
            f.create_dataset('{0}/Spres'.format(self.tag), data=self.s_pres)
        if 'q_dm' in keys:
            if 'Qdm' in fk: del f['{0}/Qdm'.format(self.tag)]
            f.create_dataset('{0}/Qdm'.format(self.tag), data=self.q_dm)
        if 's_dm' in keys:
            if 'Sdm' in fk: del f['{0}/Sdm'.format(self.tag)]
            f.create_dataset('{0}/Sdm'.format(self.tag), data=self.s_dm)
        if 'q_star' in keys:
            if 'Qstar' in fk: del f['{0}/Qstar'.format(self.tag)]
            f.create_dataset('{0}/Qstar'.format(self.tag), data=self.q_star)
        if 's_star' in keys:
            if 'Sstar' in fk: del f['{0}/Sstar'.format(self.tag)]
            f.create_dataset('{0}/Sstar'.format(self.tag), data=self.s_star)
        # Eigenvector profiles
        if 'Iv_gas' in keys:
            if 'Veigen_gas_pro' in fk: del f['{0}/Veigen_gas_pro'.format(self.tag)]
            f.create_dataset('{0}/Veigen_gas_pro'.format(self.tag), data=self.Iv_gas)
        if 'Iv_ghot' in keys:
            if 'Veigen_gas_hot_pro' in fk: del f['{0}/Veigen_gas_hot_pro'.format(self.tag)]
            f.create_dataset('{0}/Veigen_gas_hot_pro'.format(self.tag), data=self.Iv_ghot)
        if 'Iv_temp' in keys:
            if 'Veigen_temp_pro' in fk: del f['{0}/Veigen_temp_pro'.format(self.tag)]
            f.create_dataset('{0}/Veigen_temp_pro'.format(self.tag), data=self.Iv_temp)
        if 'Iv_pres' in keys:
            if 'Veigen_pres_pro' in fk: del f['{0}/Veigen_pres_pro'.format(self.tag)]
            f.create_dataset('{0}/Veigen_pres_pro'.format(self.tag), data=self.Iv_pres)
        if 'Iv_dm' in keys:
            if 'Veigen_dm_pro' in fk: del f['{0}/Veigen_dm_pro'.format(self.tag)]
            f.create_dataset('{0}/Veigen_dm_pro'.format(self.tag), data=self.Iv_dm)
        if 'Iv_star' in keys:
            if 'Veigen_star_pro' in fk: del f['{0}/Veigen_star_pro'.format(self.tag)]
            f.create_dataset('{0}/Veigen_star_pro'.format(self.tag), data=self.Iv_star)
        # Cumulative eigenvectors
        if 'Iv_gas_200' in keys:
            if 'Veigen_gas_200' in fk: del f['{0}/Veigen_gas_200'.format(self.tag)]
            f.create_dataset('{0}/Veigen_gas_200'.format(self.tag), data=self.Iv_gas_200)
        if 'Iv_gas_500' in keys:
            if 'Veigen_gas_500' in fk: del f['{0}/Veigen_gas_500'.format(self.tag)]
            f.create_dataset('{0}/Veigen_gas_500'.format(self.tag), data=self.Iv_gas_500)
        if 'Iv_gas_vir' in keys:
            if 'Veigen_gas_vir' in fk: del f['{0}/Veigen_gas_vir'.format(self.tag)]
            f.create_dataset('{0}/Veigen_gas_vir'.format(self.tag), data=self.Iv_gas_vir)
        if 'Iv_dm_200' in keys:
            if 'Veigen_dm_200' in fk: del f['{0}/Veigen_dm_200'.format(self.tag)]
            f.create_dataset('{0}/Veigen_dm_200'.format(self.tag), data=self.Iv_dm_200)
        if 'Iv_dm_500' in keys:
            if 'Veigen_dm_500' in fk: del f['{0}/Veigen_dm_500'.format(self.tag)]
            f.create_dataset('{0}/Veigen_dm_500'.format(self.tag), data=self.Iv_dm_500)
        if 'Iv_dm_vir' in keys:
            if 'Veigen_dm_vir' in fk: del f['{0}/Veigen_dm_vir'.format(self.tag)]
            f.create_dataset('{0}/Veigen_dm_vir'.format(self.tag), data=self.Iv_dm_vir)
        if 'Iv_st_200' in keys:
            if 'Veigen_star_200' in fk: del f['{0}/Veigen_star_200'.format(self.tag)]
            f.create_dataset('{0}/Veigen_star_200'.format(self.tag), data=self.Iv_st_200)
        if 'Iv_st_500' in keys:
            if 'Veigen_star_500' in fk: del f['{0}/Veigen_star_500'.format(self.tag)]
            f.create_dataset('{0}/Veigen_star_500'.format(self.tag), data=self.Iv_st_500)
        if 'Iv_st_vir' in keys:
            if 'Veigen_star_vir' in fk: del f['{0}/Veigen_star_vir'.format(self.tag)]
            f.create_dataset('{0}/Veigen_star_vir'.format(self.tag), data=self.Iv_st_vir)
        f.close()
        return
    ...
    def _spectrum_model(self, p0, spectrum, MODE):
        ...
        if T < self.APEC_temperatures[itemp]: itemp -= 1
        if itemp < 0: itemp = 0
        elif itemp > len(self.APEC_temperatures) - 2: itemp = len(self.APEC_temperatures) - 2
        ...
        model  = 10.0 ** (self.Enorm + D) * (H + He + Z * (C + N + O + Ne + Mg + Si + S + Ca + Fe))
    ...
    def _calculate_photon_conversion_factor(self, Tint=1.0e6):
        ...
        self.photon_conv  = Absorption * self.APEC_area * Tint \
                            / (4.0 * np.pi * Dlum ** 2.0) / ct.kev_2_erg
    ...
    def _wabs(self, E, col):
        ...
        EX    = np.array([0.100, 0.284, 0.400, 0.532, 0.707, 0.867, 1.303,
                          1.840, 2.471, 3.210, 4.038, 7.111, 8.331, 10.000])
        C0    = np.array([17.30, 34.60, 78.10, 71.40, 95.50, 308.9, 120.6,
                          141.3, 202.7, 342.7, 352.2, 433.9, 629.0, 701.2])
        C1    = np.array([608.1, 267.9, 18.80, 66.80, 145.8, -380.6, 169.3,
                          146.8, 104.7, 18.70, 18.70, -2.40, 30.90, 25.20])
        C2    = np.array([-2150.0, -476.1, 4.3, -51.4, -61.1, 294.0, -47.7,
...
def _check_points_unique(p):
    ...
    if len(x_non_unique) > 0 and len(y_non_unique) > 0 and len(z_non_unique) > 0:
        ...
        if len(idx) > 0:
            for i in idx: p[i] += np.random.rand(3) * 1.0e-6 * ct.Mpc_cm
